...
function printRouteImports(route: Route, lazy = true): string {
  ...
  return lines.sort((a, b) => a.includes('const') === b.includes('const') ? 0 : a.includes('const') ? 1 : -1).join('\n')
}
...
function printRoute(route: Route, lazy = true): string {
  ...
  if (route.lazy && lazy) {
    const loadingComponent = route.loading ? `${route.name}Loading` : `Loading source="Router ${route.name}"`
  ...
  if (route.page) {
    ...
    lines.push('<Route index element={<NotFound  />} />')
  }
  if (route.children) {
...
${routeOutput.includes('<Loading') ? 'import Loading from \'~components/common/Loading\'' : ''}
${routeImportsOutput}
